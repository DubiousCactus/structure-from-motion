...
@author: NKrvavica (https://github.com/NKrvavica/fqs)
"""
...
def cubic_roots(p):
    ...
    if p.shape[1] != 4:
        raise ValueError(
            "Expected 3rd order polynomial with 4 coefficients, got {:d}.".format(
                p.shape[1]
            )
        )
...
def quartic_roots(p):
    ...
    if p.shape[1] != 5:
        raise ValueError(
            "Expected 4th order polynomial with 5 coefficients, got {:d}.".format(
                p.shape[1]
            )
        )
